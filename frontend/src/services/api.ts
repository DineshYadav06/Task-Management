...
export const wsApi = {
  list: async (org_id?: number) => (await api.get<Workspace[]>('/workspaces', { params: { org_id } })).data,
...
export const projectApi = {
  list: async (workspace_id?: number) => (await api.get<Project[]>('/projects', { params: { workspace_id } })).data,
...
export const sprintApi = {
  list: async (project_id?: number) => (await api.get<Sprint[]>('/sprints', { params: { project_id } })).data,
...
export const taskApi = {
  list: async (params?: { project_id?: number; assignee_id?: number; sprint_id?: number; column_id?: number; status?: string; page?: number }) => (await api.get('/tasks', { params })).data,
  create: async (data: Partial<Task>) => (await api.post<Task>('/tasks', data)).data,
...
export const kanbanApi = {
  getColumns: async (project_id: number) => (await api.get<BoardColumn[]>('/kanban/columns', { params: { project_id } })).data,
...
export const timeApi = {
  ...
  getLogs: async (task_id?: number) => (await api.get<TimeLog[]>('/timetracking/logs', { params: { task_id } })).data,
  getTimesheet: async (week_start: string) => (await api.get('/timetracking/timesheet', { params: { week_start } })).data,
};

export const notifApi = {
  list: async (unread_only = false) => (await api.get<NotificationItem[]>('/notifications', { params: { unread_only } })).data,
